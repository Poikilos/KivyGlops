...
const vec4 cons36 = vec4(1.000000, 1.000000, 1.000000, 0.005000);
uniform float unf47;
uniform float unf50;
uniform vec3 unf56;
const float cons66 = float(1.000000);
uniform vec4 unf67;
const float cons70 = float(1.000000);
uniform float unf71;
uniform float unf80;
uniform float unf81;
uniform float unf82;
...
void main(void)
{
	...
	set_rgb_zero(tmp7);
	set_rgb_zero(tmp8);
	lamp_visibility_other(varposition, unf10, tmp11, tmp12, tmp13);
	lamp_falloff_invsquare(unf14, tmp12, tmp16);
	lamp_visibility_clamp(tmp16, tmp18);
	shade_inp(tmp4, tmp11, tmp21);
	shade_visifac(tmp21, tmp18, unf24, tmp25);
	shade_mul_value_v3(unf26, unf27, tmp28);
	shade_mul_value(tmp25, vec4(tmp28, 1.0), tmp31);
	mtex_value_invert(cons32, tmp33);
	mix_mult(tmp33, tmp31, cons36, tmp37);
	mtex_value_invert(tmp33, tmp39);
	shade_madd(vec4(tmp7, 1.0), tmp37, vec4(tmp2, 1.0), tmp43);
	shade_cooktorr_spec(tmp4, tmp11, tmp6, unf47, tmp48);
	shade_spec_t(tmp39, unf50, tmp18, tmp48, tmp53);
	shade_add_spec(tmp53, tmp28, unf56, tmp57);
	shade_add_clamped(vec4(tmp8, 1.0), vec4(tmp57, 1.0), tmp60);
	shade_clamp_positive(tmp60, tmp62);
	shade_clamp_positive(tmp43, tmp64);
	shade_maddf(tmp64, cons66, unf67, tmp68);
	alpha_spec_correction(tmp62.rgb, cons70, unf71, tmp72);
	shade_add(tmp68, tmp62, tmp75);
	mtex_alpha_to_col(tmp75, tmp72, tmp78);
	shade_mist_factor(varposition, unf80, unf81, unf82, unf83, unf84, tmp85);
	mix_blend(tmp85, tmp78, unf88, tmp89);
	mtex_alpha_to_col(tmp89, tmp72, tmp92);
	output_node(tmp92, tmp72, tmp95);
	linearrgb_to_srgb(tmp95, tmp97);
