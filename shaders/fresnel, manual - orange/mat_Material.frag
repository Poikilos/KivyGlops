...
varying vec2 var1;
varying vec4 var2;
const vec3 cons17 = vec3(0.000000, 0.000000, 1.000000);
uniform sampler2D samp0;
const vec4 cons25 = vec4(0.950000, 0.300000, 0.001649, 1.000000);
const vec4 cons26 = vec4(0.000000, 0.000000, 0.000000, 1.000000);
uniform sampler2D samp1;
const float cons35 = float(1.000000);
uniform vec3 unf46;
uniform float unf50;
uniform float unf60;
uniform float unf62;
uniform vec3 unf63;
const float cons68 = float(1.000000);
const vec4 cons72 = vec4(1.000000, 1.000000, 1.000000, 0.005000);
uniform float unf83;
uniform float unf86;
uniform vec3 unf92;
uniform vec3 unf98;
uniform float unf102;
uniform float unf112;
uniform float unf114;
uniform vec3 unf115;
const float cons120 = float(1.000000);
const vec4 cons124 = vec4(1.000000, 1.000000, 1.000000, 0.005000);
uniform float unf135;
uniform float unf138;
uniform vec3 unf144;
const float cons154 = float(1.000000);
uniform vec4 unf155;
const float cons161 = float(1.000000);
uniform float unf164;
uniform float unf165;
uniform float unf166;
uniform float unf167;
uniform float unf168;
uniform vec4 unf172;
const float cons181 = float(1.000000);
const float cons184 = float(1.000000);
...
void main(void)
{
	...
	normal(tmp12, cons17, tmp18, tmp19);
	valtorgb(tmp19, samp0, tmp22, tmp23);
	mix_blend((tmp22.r + tmp22.g + tmp22.b) / 3.0, cons25, cons26, tmp27);
	texture_image(tmp11, samp1, tmp30, tmp31, tmp32);
	srgb_to_linearrgb(tmp31, tmp34);
	mix_add(cons35, tmp34, tmp27, tmp38);
	shade_norm(facingnormal, tmp40);
	shade_view(varposition, tmp42);
	set_rgb_zero(tmp43);
	set_rgb_zero(tmp44);
	lamp_visibility_other(varposition, unf46, tmp47, tmp48, tmp49);
	lamp_falloff_invsquare(unf50, tmp48, tmp52);
	lamp_visibility_clamp(tmp52, tmp54);
	shade_inp(tmp40, tmp47, tmp57);
	shade_visifac(tmp57, tmp54, unf60, tmp61);
	shade_mul_value_v3(unf62, unf63, tmp64);
	shade_mul_value(tmp61, vec4(tmp64, 1.0), tmp67);
	mtex_value_invert(cons68, tmp69);
	mix_mult(tmp69, tmp67, cons72, tmp73);
	mtex_value_invert(tmp69, tmp75);
	shade_madd(vec4(tmp43, 1.0), tmp73, tmp38, tmp79);
	shade_cooktorr_spec(tmp40, tmp47, tmp42, unf83, tmp84);
	shade_spec_t(tmp75, unf86, tmp54, tmp84, tmp89);
	shade_add_spec(tmp89, tmp64, unf92, tmp93);
	shade_add_clamped(vec4(tmp44, 1.0), vec4(tmp93, 1.0), tmp96);
	lamp_visibility_other(varposition, unf98, tmp99, tmp100, tmp101);
	lamp_falloff_invsquare(unf102, tmp100, tmp104);
	lamp_visibility_clamp(tmp104, tmp106);
	shade_inp(tmp40, tmp99, tmp109);
	shade_visifac(tmp109, tmp106, unf112, tmp113);
	shade_mul_value_v3(unf114, unf115, tmp116);
	shade_mul_value(tmp113, vec4(tmp116, 1.0), tmp119);
	mtex_value_invert(cons120, tmp121);
	mix_mult(tmp121, tmp119, cons124, tmp125);
	mtex_value_invert(tmp121, tmp127);
	shade_madd(tmp79, tmp125, tmp38, tmp131);
	shade_cooktorr_spec(tmp40, tmp99, tmp42, unf135, tmp136);
	shade_spec_t(tmp127, unf138, tmp106, tmp136, tmp141);
	shade_add_spec(tmp141, tmp116, unf144, tmp145);
	shade_add_clamped(tmp96, vec4(tmp145, 1.0), tmp148);
	shade_clamp_positive(tmp148, tmp150);
	shade_clamp_positive(tmp131, tmp152);
	shade_maddf(tmp152, cons154, unf155, tmp156);
	shade_add(tmp156, tmp150, tmp159);
	mtex_alpha_to_col(tmp159, cons161, tmp162);
	shade_mist_factor(varposition, unf164, unf165, unf166, unf167, unf168, tmp169);
	mix_blend(tmp169, tmp162, unf172, tmp173);
	shade_alpha_opaque(tmp173, tmp175);
	vec_math_sub(tmp175.rgb, tmp150.rgb, tmp178, tmp179);
	mtex_alpha_to_col(vec4(tmp178, 1.0), cons181, tmp182);
	output_node(tmp182, cons184, tmp185);
	linearrgb_to_srgb(tmp185, tmp187);
