...
uniform float unf42;
uniform vec3 unf43;
const float cons48 = float(1.000000);
const vec4 cons52 = vec4(1.000000, 1.000000, 1.000000, 0.005000);
uniform float unf63;
uniform float unf66;
uniform vec3 unf72;
const float cons82 = float(1.000000);
uniform vec4 unf83;
const float cons89 = float(1.000000);
uniform float unf92;
uniform float unf93;
uniform float unf94;
uniform float unf95;
uniform float unf96;
uniform vec4 unf100;
const float cons105 = float(1.000000);
const float cons108 = float(1.000000);
...
void main(void)
{
	...
	set_rgb(unf16, tmp17);
	shade_view(varposition, tmp19);
	vec_math_normalize(tmp12, tmp21, tmp22);
	set_rgb_zero(tmp23);
	set_rgb_zero(tmp24);
	lamp_visibility_other(varposition, unf26, tmp27, tmp28, tmp29);
	lamp_falloff_invsquare(unf30, tmp28, tmp32);
	lamp_visibility_clamp(tmp32, tmp34);
	shade_inp(tmp21, tmp27, tmp37);
	shade_visifac(tmp37, tmp34, unf40, tmp41);
	shade_mul_value_v3(unf42, unf43, tmp44);
	shade_mul_value(tmp41, vec4(tmp44, 1.0), tmp47);
	mtex_value_invert(cons48, tmp49);
	mix_mult(tmp49, tmp47, cons52, tmp53);
	mtex_value_invert(tmp49, tmp55);
	shade_madd(vec4(tmp23, 1.0), tmp53, vec4(tmp17, 1.0), tmp59);
	shade_cooktorr_spec(tmp21, tmp27, tmp19, unf63, tmp64);
	shade_spec_t(tmp55, unf66, tmp34, tmp64, tmp69);
	shade_add_spec(tmp69, tmp44, unf72, tmp73);
	shade_add_clamped(vec4(tmp24, 1.0), vec4(tmp73, 1.0), tmp76);
	shade_clamp_positive(tmp76, tmp78);
	shade_clamp_positive(tmp59, tmp80);
	shade_maddf(tmp80, cons82, unf83, tmp84);
	shade_add(tmp84, tmp78, tmp87);
	mtex_alpha_to_col(tmp87, cons89, tmp90);
	shade_mist_factor(varposition, unf92, unf93, unf94, unf95, unf96, tmp97);
	mix_blend(tmp97, tmp90, unf100, tmp101);
	shade_alpha_opaque(tmp101, tmp103);
	mtex_alpha_to_col(tmp103, cons105, tmp106);
	output_node(tmp106, cons108, tmp109);
	linearrgb_to_srgb(tmp109, tmp111);
