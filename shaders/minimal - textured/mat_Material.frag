...
uniform float unf47;
uniform vec3 unf48;
const float cons53 = float(1.000000);
const vec4 cons57 = vec4(1.000000, 1.000000, 1.000000, 0.005000);
uniform float unf68;
uniform float unf71;
uniform vec3 unf77;
const float cons87 = float(1.000000);
uniform vec4 unf88;
const float cons94 = float(1.000000);
uniform float unf97;
uniform float unf98;
uniform float unf99;
uniform float unf100;
uniform float unf101;
uniform vec4 unf105;
const float cons110 = float(1.000000);
const float cons113 = float(1.000000);
...
void main(void)
{
	...
	shade_view(varposition, tmp24);
	vec_math_normalize(tmp12, tmp26, tmp27);
	set_rgb_zero(tmp28);
	set_rgb_zero(tmp29);
	lamp_visibility_other(varposition, unf31, tmp32, tmp33, tmp34);
	lamp_falloff_invsquare(unf35, tmp33, tmp37);
	lamp_visibility_clamp(tmp37, tmp39);
	shade_inp(tmp26, tmp32, tmp42);
	shade_visifac(tmp42, tmp39, unf45, tmp46);
	shade_mul_value_v3(unf47, unf48, tmp49);
	shade_mul_value(tmp46, vec4(tmp49, 1.0), tmp52);
	mtex_value_invert(cons53, tmp54);
	mix_mult(tmp54, tmp52, cons57, tmp58);
	mtex_value_invert(tmp54, tmp60);
	shade_madd(vec4(tmp28, 1.0), tmp58, tmp22, tmp64);
	shade_cooktorr_spec(tmp26, tmp32, tmp24, unf68, tmp69);
	shade_spec_t(tmp60, unf71, tmp39, tmp69, tmp74);
	shade_add_spec(tmp74, tmp49, unf77, tmp78);
	shade_add_clamped(vec4(tmp29, 1.0), vec4(tmp78, 1.0), tmp81);
	shade_clamp_positive(tmp81, tmp83);
	shade_clamp_positive(tmp64, tmp85);
	shade_maddf(tmp85, cons87, unf88, tmp89);
	shade_add(tmp89, tmp83, tmp92);
	mtex_alpha_to_col(tmp92, cons94, tmp95);
	shade_mist_factor(varposition, unf97, unf98, unf99, unf100, unf101, tmp102);
	mix_blend(tmp102, tmp95, unf105, tmp106);
	shade_alpha_opaque(tmp106, tmp108);
	mtex_alpha_to_col(tmp108, cons110, tmp111);
	output_node(tmp111, cons113, tmp114);
	linearrgb_to_srgb(tmp114, tmp116);
