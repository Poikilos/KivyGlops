...
settings['templates']['actor']['land_speed'] = \
...
def get_near_line_info_xz(a, b, c):
    ...
    kMinSegmentLenSquared = 0.00000001
...
def is_in_triangle_coords(px, py, p0x, p0y, p1x, p1y, p2x, p2y):
    '''
    IsInTriangle_Barymetric (within Epsilon)
    '''
...
class PyGlop:
        ...
        if sv is not None:
            ...
            if phr is not None:
                ...
                print("[ PyGlop ] WARNING in apply_vertex_offset:"
                      " hit_radius not applied since missing.")
        else:
            print("[ PyGlop ] WARNING in apply_vertex_offset: "
    ...
    def get_texture_diffuse_path(self):
        ...
        try:
            ...
            if not os.path.exists(result):
                # try to combine with glop's directory:
    ...
    def emit_yaml(self, lines, min_tab_string):
        ...
        while c_i < len(vf):
            ...
            lines.append(min_tab_string + component_name +
                         ".offset:" +
    ...
    def append_wobject(self, this_wobject, pivot_to_g_enable=True):
        ...
        for channel_subindex in range(0, channel_count):
            ...
            # (other than negative) to work with a unified shader
        ...
        for key in this_wobject.face_dicts:
            ...
            for this_wobject_this_face in this_face_list:
                ...
                while vertexinfo_index < len(this_wobject_this_face):
                    ...
                    if len(v) > 3:
                        # use extended vertex info (color) from
                        # nonstandard obj file
                        abs_index = 0
                        for element_index in range(4, len(v)):
                            vertex_components[self.COLOR_OFFSET
                                              + abs_index] = \
                                                  v[element_index]
                            abs_index += 1
                    else:
                ...
                vertexinfo_index = 0
                sFaceVI = 0
                # ^ required for tracking faces with less than 3 vertices
                ...
                # face_vertex_list = list()  # in case verts are out
                # of order, prevent tesselation from connecting
                # wrong verts
                while vertexinfo_index < len(this_wobject_this_face):
                    # face_vertex_list.append(dest_vertex_index)
                    if vertexinfo_index == 2:
                        # OK to assume dest vertices are in order,
                        # since just created them (should work even
                        # if source vertices are not in order)
                        tri = [
...
class PyGlops:
    ...
    def give_item_by_keyword_to_player_number(self, player_number,
        ...
        cause player to obtain the item found first by keyword, then
        hide the item (overrides object bump code)
        '''
    ...
    def give_item_index_to_player_number(self, player_number,
        ...
        pre_commands can be either None (to imply default "hide") or a
        string containing semicolon-separated commands that will occur
        before obtain
        '''
        result = False
    ...
    def _run_command(self, command, bumpable_index, bumper_index,
        ...
        if command == "hide":
            ...
            print(
                "Glop named {} attempted an unknown glop command"
                " (in bump event): {}"
                "".format(self.glops[bumpable_index].name,
                          command)
            )
    ...
    def _internal_bump_glop(self, bumpable_index, bumper_index_or_None):
        ...
        if bumper_index is not None:
            if bumper_index not in bumped.state['in_range_indices']:
                ...
                print("[ PyGlops ] WARNING in _internal_bump_glop: '"
                      + bumped.name + "' is already"
                      " being bumped by '" + str(bgn))
        ...
        if dgPjD is not None:
            ...
            #if bumper is not None:
            #    print("[ debug only ] projectile bumped by object " + str(bgn))
            #    print("[ debug only ]    hit_radius:" + str(bumper.properties['hit_radius']))
            #    if bumper.properties['hitbox'] is not None:
            #        print("[ debug only ]   hitbox: "
            #              + str(bumper.properties['hitbox']))
            #    else:
            #        print("bumped.item_dict does not contain 'bump'")
        elif bumped.item_dict is not None:
            bad_flag = "(projectile_dict)"
            this_flag = "<unexpected scenario>"
            if "bump" in bumped.item_dict:
                if bumped.properties['bump_enable']:
                    #if "rock" in bumped.name.lower():
                        # debug only
                    if dgPjD is not None:
                        this_flag = bad_flag
                    elif bumped.item_dict is not None and \
                       'as_projectile' in bumped.item_dict:
                    ...
                    if bumper is not None:
                        if (bumped.item_dict is None) or \
                           ('owner_key' not in bumped.item_dict) or \
                           (bumped.item_dict['owner_key'] is None):
                            if bumped.item_dict["bump"] is not None:
                                # self._run_semicolon_separated_commands(bumped.item_dict["bump"], bumpable_index, bumper_index);
                                commands = bumped.item_dict["bump"].split(";")
                                ...
                                if get_verbose_enable():
                                    print("[ PyGlops ] bumped.item_dict['bump'] is None")
                        else:
                            if get_verbose_enable():
                                bgn_msg = "'" + str(bgn) + "'"
                                if bgn is None:
                                    bgn_msg = 'world'
                                print("[ PyGlops ] " + bgn_msg + \
                                      " is not bumping into '" + \
                                      bumped.name + "' since it was " + \
                                      "already obtained by [" +
                                      str(bumped.item_dict['owner_key']) + \
                                      "] " + \
                                      str(self.glops[bumped.item_dict['owner_key']].name))
                    else:
                        if get_verbose_enable():
                ...
                if get_verbose_enable():
                    print("[ PyGlops ] bumped.item_dict does not contain 'bump'")
        else:
            if get_verbose_enable():
...
#                              " could not get item_glop")
        ...
        if user_glop is not None:
            ...
            if fires_glops is not None:
                for fires_glop in fires_glops:
                    ...
                    fired_glop.projectile_dict['owner_key'] = user_glop.glop_index
    ...
    def on_update_glops(self):
        # print("INFO: subclass of PyGlops"
