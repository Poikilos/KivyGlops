...
attribute vec3  vNormal; //set by vertex (named by vertex_format)
attribute vec4  vColor; //TODO: oops noone set this (neither kivy nor nskrypnik [C:\Kivy-1.8.0-py3.3-win32\kivy\examples\3Drendering\simple.glsl doesn't even mention the variable ])--vertex_format should name it. No wonder having no texture (or multiplying by vColor) makes object invisible. 
attribute vec2  vTexCoord0; //set by vertex (named by vertex_format)
...
varying vec4 frag_color; //set by vertex shader below (according to vColor)
varying vec2 uv_vec; 
