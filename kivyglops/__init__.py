...
class KivyGlop(PyGlop):  # formerly KivyGlop(Widget, PyGlop)
    ...
    def prepare_canvas(self, use_meshes=None, axes_widget_index=-1):
        '''
        Keyword arguments:
        use_meshes -- Set the list of meshes to override `[self._mesh]`.
        axes_widget_index -- The index in use_meshes that is the XYZ
                             axes widget that shows this model's
                             orientation.
        '''
...
class KivyGlops(PyGlops):
    ...
    def update(self):
        ...
        for motivated_index in range(len(self.glops)):
            ...
            choice_local_vel_mult = [0., 0., 0.]
            # ^ 1.0 is max joystick tilt: normally [0],[2]
            ...
            try:
                ...
                raise KeyError("(FATAL) state template was not applied"
                    " to glop named {} from file '{}' but"
                    " physics-enabled glop must always have"
                    " velocity".format(m_glop.name, m_glop.source_path)
                )
            ar_enable = False
            ...
            if mgp['physics_enable']:
                ...
                src_pos = m_glop.get_pos()
                dst_pos = (
                    src_pos[0] + mgsv[0] * got_frame_delay,
                ...
                debugKey = m_glop.name
                if debugKey == "Player 1":
                    debugKey = "Player"
                if debugKey not in debug_dict:
                    debug_dict[debugKey] = {}
                debug_dict[debugKey]['pos'] = m_glop.get_pos()
                if not mgs['on_ground_enable']:
            ...
            if p1_enable:
                if 'Player' not in dd:
                    dd['Player'] = {}
                dd['Player']['check_pos_enable'] = check_pos_enable
            if check_pos_enable and mgp['clip_enable']:
                ...
                if walkmesh_enable:
                    if len(self._walkmeshes) > 0:
                        ...
                        if (motivated_index
                            ...
                            dd['Player']['feet_y'] = walk_info['feet_y']
                ...
                if const_ground_enable:
                    ...
                    if corrected_pos[1] > walk_info['pos'][1]:
                        ...
                        # Hovering (previous if-else flags for physics)
                        pass
...
class KivyGlopsWindow(ContainerForm):  # formerly a subclass of Widget
    ...
    def update_glsl(self, *largs):
        ...
        debug_dict['View']['camera xyz: '] = fixed_width(
                self.scene.camera_glop._t_ins.xyz,
                6,
                " "
        )
