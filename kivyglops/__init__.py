        ...
        for detectable_i_i in range(0, len(self._bumpable_indices)):
            ...
            if bumpable_index is not None:
                ...
                for rel in self.glops[bumpable_index].state["links"]:
                    if rel["r_type"] == "carry":
                        self.glops[bumpable_index]._t_ins.x = rel["state"]["parent_glop"]._t_ins.x
            ...
            if not motivated_glop.state['at_rest_enable']:
                ...
                if motivated_glop.physics_enable:
                    ...
                    if motivated_glop._cached_floor_y is not None:
                        if motivated_glop._t_ins.y - motivated_glop.properties['hit_radius'] - kEpsilon > motivated_glop._cached_floor_y:
                    ...
                    if stop_this_bumpable_enable:  # HIT GROUND etc
                        #TODO: optionally, such as for bottom-heavy items: motivated_glop._r_ins_x.angle = 0.
                        #if motivated_glop.z_velocity > kEpsilon:
                        if (motivated_glop.y_velocity < 0.0 - (kEpsilon + motivated_glop.properties['hit_radius'])):
                        ...
                        if motivated_glop.item_dict is not None:
                            ...
                            if motivated_glop.item_dict["state"].get("owner_key") is not None:
                                del motivated_glop.item_dict["state"]["owner_key"]

                        motivated_glop._t_ins.y = motivated_glop._cached_floor_y + motivated_glop.properties['hit_radius']
                           ...
                           motivated_glop.z_velocity != 0.0:
                            if get_verbose_enable():
                                print("[ KivyGlops ] stopped glop {" + \
                                      "hit_radius:" + \
                                      str(motivated_glop.properties['hit_radius']) + \
                    ...
                    if not motivated_glop.state['at_rest_enable']:
                        ...
                        motivated_glop._t_ins.z += motivated_glop.z_velocity * got_frame_delay
                        if got_frame_delay > 0.0:
                            #print("[ KivyGlops ] (verbose message) GRAVITY AFFECTED:"+str(motivated_glop._t_ins.y)+" += "+str(motivated_glop.y_velocity))
                ...
                if rotation_multiplier_y != 0.0:
                    ...
                    motivated_glop._r_ins_y.angle += delta_y
                    #origin_distance = math.sqrt(motivated_glop._t_ins.x*motivated_glop._t_ins.x + motivated_glop._t_ins.z*motivated_glop._t_ins.z)
                    #motivated_glop._t_ins.x -= origin_distance * math.cos(delta_y)
                    #motivated_glop._t_ins.z -= origin_distance * math.sin(delta_y)
                ...
                if moving_x != 0.0 or moving_y != 0.0 or moving_z != 0.0:
                    ...
                    position_change[0] = lupf*moving_r_multiplier * math.cos(motivated_glop._r_ins_y.angle+moving_theta+math.radians(-90))
                    position_change[1] = lupf*moving_y
                    position_change[2] = lupf*moving_r_multiplier * math.sin(motivated_glop._r_ins_y.angle+moving_theta+math.radians(-90))

                    # if (motivated_glop._t_ins.x + move_by_x > self._world_cube.get_max_x()):
                    #     move_by_x = self._world_cube.get_max_x() - motivated_glop._t_ins.x
                    #     print(str(motivated_glop._t_ins.x)+" of max_x:"+str(self._world_cube.get_max_x()))
                    # if (motivated_glop._t_ins.z + move_by_z > self._world_cube.get_max_z()):
                    #     move_by_z = self._world_cube.get_max_z() - motivated_glop._t_ins.z
                    #     print(str(motivated_glop._t_ins.z)+" of max_z:"+str(self._world_cube.get_max_z()))
                    # if (motivated_glop._t_ins.x + move_by_x < self._world_cube.get_min_x()):
                    #     move_by_x = self._world_cube.get_min_x() - motivated_glop._t_ins.x
                    #     print(str(motivated_glop._t_ins.x)+" of max_x:"+str(self._world_cube.get_max_x()))
                    # if (motivated_glop._t_ins.z + move_by_z < self._world_cube.get_min_z()):
                    #     move_by_z = self._world_cube.get_min_z() - motivated_glop._t_ins.z
                    #     print(str(motivated_glop._t_ins.z)+" of max_z:"+str(self._world_cube.get_max_z()))

                    #print(str(motivated_glop._t_ins.x)+","+str(motivated_glop._t_ins.z)+" each coordinate should be between matching one in "+str(self._world_cube.get_min_x())+","+str(self._world_cube.get_min_z())+" and "+str(self._world_cube.get_max_x())+","+str(self._world_cube.get_max_z()))
                    #print(str( (motivated_glop._t_ins.x, motivated_glop._t_ins.y, motivated_glop._t_ins.z) )+" each coordinate should be between matching one in "+str(self.world_boundary_min)+" and "+str(self.world_boundary_max))

                #for axis_index in range(0,3):
                if position_change[0] is not None:
                    motivated_glop._t_ins.x += position_change[0]
                if position_change[1] is not None:
                    motivated_glop._t_ins.y += position_change[1]
                if position_change[2] is not None:
                    motivated_glop._t_ins.z += position_change[2]
                ...
                # else:
                #     self.camera_glop._t_ins.x += self.lupf * moving_x
                #     self.camera_glop._t_ins.z += self.lupf * moving_z
                if theta is not None:
                #if lupf is not None:
                    motivated_glop._r_ins_y.angle = theta  # TODO: use land_radians_per_frame instead of turning instantly
                #    motivated_glop._t_ins.x += moving_x
                #    motivated_glop._t_ins.z += moving_z
                #else:
                #    print("[ KivyGlops ] ERROR in update: theta was set for unit, but engine forgot to set lupf")
    ...
    def add_glop(self, this_glop, set_visible_enable=None):
        ...
        try:
            ...
            if self.scene.glops is None:
                self.scene.glops = []
            if ((self.scene.selected_glop_index is None)
                    or (self.scene.selected_glop_index < 0)):
